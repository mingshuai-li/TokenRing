...
def set_seq_parallel_pg(
    ...
    if use_ulysses_low:
        for dp_rank in range(dp_degree):
            ...
            for i in range(num_ring_pgs):
                ...
                groupQ = torch.distributed.new_group(ring_ranks)
                groupOut = torch.distributed.new_group(ring_ranks)
                if rank in ring_ranks:
                    ring_pg = group
                    ring_Q_pg = groupQ
                    ring_Out_pg = groupOut
    ...
    PROCESS_GROUP.RING_Q = ring_Q_pg
    PROCESS_GROUP.RING_OUT = ring_Out_pg
